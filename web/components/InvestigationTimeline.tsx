        ...
        {steps.map((step, i) => {
          ...
          return (
            <li key={`${step.tool}-${i}`} className="relative">
              <span className="absolute -left-[36px] top-3 flex h-7 w-7 items-center justify-center rounded-full border border-border bg-surface shadow-sm">
                <Icon size={13} strokeWidth={1.75} className="text-text" />
              </span>
              <div className="rounded-md border border-border bg-surface px-4 py-3 shadow-[0_1px_0_rgba(0,0,0,0.02)]">
                <div className="flex items-baseline justify-between gap-3">
                  <code className="text-[12px] font-medium text-text">{step.tool}</code>
                  <time className="tabular rounded-sm bg-surface-alt px-1.5 py-0.5 font-mono text-[11px] text-text-muted">
                    {formatTime(step.timestamp)}
                  </time>
                </div>
                <p className="mt-1.5 text-[13px] leading-relaxed text-text/90">{step.summary}</p>
                {step.cypher_used && (
                  <pre className="mt-2 overflow-x-auto rounded-sm border border-border/60 bg-surface-alt px-3 py-2 font-mono text-[11px] leading-relaxed text-text/90">
                    {step.cypher_used}
                  </pre>
                )}
